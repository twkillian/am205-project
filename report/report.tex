...
As expected, centers are consistently located near urban areas. Also as expected, cost decreases, albeit erratically, with the number of centers. The five center arrangement, for example, results in higher cost than the four center arrangement, but this is due to the random initialization of starting centers. Had time permitted, averaging multiple runs would have produced a smoother curve and fewer unexpected results. Nonetheless, the results are reasonable and consistent with expectations.

\subsection{Effect of Population Thresholding on Cost}
Figure \ref{fig:threshold} shows the effects of population thresholding on center placement for various population thresholds. Recall that a population threshold of $n$ means that points with population less than $n$ are treated as having a population of zero. All thresholds were tested with five centers.
...
Figure \ref{fig:weightcomp} shows the results of generating 10 clusters under four different weighting functions.
